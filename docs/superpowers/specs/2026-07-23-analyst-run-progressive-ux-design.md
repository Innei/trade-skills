...
- 运行失败 / 超时：feed 末尾追加失败条目（复用现有失败状态）。失败时后端 run state 随锁释放而清除，分段的保留由前端负责：`analystRunsStore` 在收到失败状态时保留最后一次的 sections 与 activities，卡片盖「分析未完成」标记，直到用户重跑或离开页面；此时若断线重连，中间态不再恢复（可接受）。
